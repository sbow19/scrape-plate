...
describe("Main Extension Views Landing Test", ()=>{
    ...
    test('renders the popup view as expected', () => {
        ...
        const sidePanelElement = screen.queryByRole("main", {name: "Side Panel view"});
    ...
    test('renders welcome screen content when no state', ()=>{
        render(<MyApp renderContext="popup" />);

        //Check that welcome menu renders as expected
        const welcomeMenu = screen.getByRole('heading', {name: /Welcome to Quick Scrape!/i});
        const createNewProjectButton = screen.getByRole("button", {name: /Create your first project/i});
    ...
    test('renders default footer buttons', ()=>{
        ...
        const createSchemaButton = screen.getByRole("button", {name: /create schema/i});
        const scrapeButton = screen.getByRole("button", {name: /scrape/i});
        const manageProjectsButton = screen.getByRole("button", {name: /manage projects/i});
    ...
    test('renders the side panel view as expected', () => {
        ...
        const sidePanelElement = screen.getByRole("main", {name: "Side Panel view"}); // Adjust to match text in your SidePanel component
